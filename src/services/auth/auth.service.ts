...
export class AuthService {
    ...
    async register({ registerData }: { registerData: CreateUser }) {
        ...
        if (!emailSent) {
            console.error(
                "Échec de l'envoi de l'email, l'utilisateur a été créé mais sans confirmation.",
            )
        }
    ...
    private isTokenValid({ token, hashPassword }) {
    ...
    async validateToken(token: string, userId: string): Promise<boolean> {
        try {
            ...
            console.log(
                '[VALIDATE TOKEN] Token validé et marqué comme utilisé pour userId:',
                userId,
            )
            return true
        } catch (error) {
